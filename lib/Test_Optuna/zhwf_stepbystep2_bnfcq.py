...
def main(q=4, clip_values_arg=None):
    ...
    # Final fc (1x1 conv acting as classifier) 8-bit quantization
    try:
        fc_key = None
        ...
        if fc_key is not None and torch.is_tensor(state_dict[fc_key]):
            ...
            if delta_fc == 0.0:
                ...
                wfq = torch.round(wf / delta_fc) * delta_fc
            state_dict[fc_key] = wfq
            print(f"=> fc.weight quantized to 8-bit (delta={delta_fc:.6f})")
    except Exception as e:
        print(f"=> Warning: fc quantization skipped/failed: {e}")
    ...
    # After adder quantization (which may have fused bias into BN.running_mean),
    # quantize all BatchNorm parameters to 8-bit.
    print("=> Quantizing all BatchNorm parameters to 8-bit")
    for k in list(state_dict.keys()):
        # target BN parameter keys: contain '.bn' and end with common BN suffixes
        if '.bn' in k and (k.endswith('.weight') or k.endswith('.bias') or k.endswith('.running_mean') or k.endswith('.running_var')):
            try:
                ...
                if delta_bn == 0.0:
                    ...
                    tq = torch.round(t / delta_bn) * delta_bn
                state_dict[k] = tq
                print(f"  Quantized {k} -> delta={delta_bn:.6f}, shape={tuple(t.shape)}")
            except Exception as e:
                print(f"  Warning: failed to quantize BN param {k}: {e}")
